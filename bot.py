...
# Đã thay đổi cấu trúc DB để chứa Hàng đợi (pending)
def load_db():
...
async def auto_backup_task():
    ...
    if data_changed and BACKUP_CHANNEL_ID:
        try:
            ...
            await channel.send(f"☁️ **SMART BACKUP V9** ({time.strftime('%H:%M')}):", file=discord.File(DB_FILE))
            data_changed = False
        except: pass
...
async def link(interaction: discord.Interaction, mc_name: str):
    ...
    if uid not in db["users"]: 
        db["users"][uid] = {"balance": 0, "mc_name": mc_name, "weapon": "Tay không", "power": 0, "level": 1, "xp": 0, "base_power": 0, "inventory": {}}
...
async def daily(interaction: discord.Interaction):
    db, uid = load_db(), str(interaction.user.id)
    if uid not in db["users"]: return await interaction.response.send_message("❌ Chưa `/link`!", ephemeral=True)
    if time.time() - db["users"][uid].get("last_daily", 0) < 86400: return await interaction.response.send_message("⏳ Đợi mai nhé!", ephemeral=True)
    reward = random.randint(20, 50)
    db["users"][uid]["last_daily"], db["users"][uid]["balance"] = time.time(), db["users"][uid].get("balance", 0) + reward
    save_db(db); await interaction.response.send_message(f"🎁 Điểm danh thành công! Nhận **${reward}**.")

# --- LỆNH MỚI: CHUYỂN TIỀN (PAY) VÀ RÚT TIỀN (QUEUE) ---
...
async def ruttien(interaction: discord.Interaction, so_tien: int):
    ...
    # Kích hoạt xử lý hàng đợi ngay lập tức (nếu server đang bật thì chạy luôn)
    await process_pending_withdrawals(bot)
